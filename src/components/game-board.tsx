...
import { renderBoard, getGridPosition, BOARD_PX, CELL_SIZE } from "@/lib/canvas-renderer";
import { BOARD_SIZE, swapPieces, findAllMatches, calculateScore } from "@/lib/game-logic";
...
export default function GameBoard({ onScoreChange, disabled = false }: GameBoardProps) {
  ...
  const { animation, animateSwap, animateMatch, animateDrop, resetAnimation } = useGameAnimation();
  ...
  const handleMove = useCallback(
    async (pos1: Position, pos2: Position) => {
      ...
      while (matches.length > 0) {
        ...
        // 드롭 계산 및 애니메이션
        const { newBoard, drops, boardWithHoles } = calculateDrops(currentBoard, matches);
        await animateDrop(drops, boardWithHoles);
    ...
    [state, disabled, actions, animateSwap, animateMatch, animateDrop, onScoreChange]
  );
  ...
  const handlePointerMove = useCallback(
    (e: React.PointerEvent) => {
      if (!state.isDragging || !state.dragStart || disabled || state.isProcessing) return;
      ...
      if (Math.abs(offsetX) > threshold || Math.abs(offsetY) > threshold) {
        const targetPos: Position =
          Math.abs(offsetX) > Math.abs(offsetY)
            ? { row: state.dragStart.row, col: state.dragStart.col + (offsetX > 0 ? 1 : -1) }
            : { row: state.dragStart.row + (offsetY > 0 ? 1 : -1), col: state.dragStart.col };
  ...
  return (
    <div className="flex flex-col items-center gap-4">
      ...
      <canvas
        ...
        style={{ background: "#1a1a2e", maxWidth: "100%", height: "auto" }}
