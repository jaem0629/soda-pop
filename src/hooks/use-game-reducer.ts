...
export function calculateDrops(
  board: Board,
  matches: Position[][]
): { newBoard: Board; drops: DropInfo[]; boardWithHoles: (PieceType | null)[][] } {
...
export function useGameReducer() {
  ...
  const actions = {
    ...
    setBoard: useCallback((board: Board) => dispatch({ type: "SET_BOARD", board }), []),
    addScore: useCallback((score: number) => dispatch({ type: "ADD_SCORE", score }), []),
    setAnimating: useCallback(
      (isAnimating: boolean) => dispatch({ type: "SET_ANIMATING", isAnimating }),
      []
    ),
    select: useCallback((pos: Position | null) => dispatch({ type: "SELECT", pos }), []),
    startDrag: useCallback((pos: Position) => dispatch({ type: "START_DRAG", pos }), []),
    updateDrag: useCallback(
      (offset: { x: number; y: number }) => dispatch({ type: "UPDATE_DRAG", offset }),
      []
    ),
    endDrag: useCallback(() => dispatch({ type: "END_DRAG" }), []),
    setProcessing: useCallback(
      (isProcessing: boolean) => dispatch({ type: "SET_PROCESSING", isProcessing }),
      []
    ),
  };

  // 이동 유효성 검사
  const isValidMove = useCallback((pos1: Position, pos2: Position): boolean => {
