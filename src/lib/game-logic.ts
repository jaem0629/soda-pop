...
// 두 조각 교환
export function swapPieces(board: Board, pos1: Position, pos2: Position): Board {
...
export function hasValidMoves(board: Board): boolean {
  for (let row = 0; row < BOARD_SIZE; row++) {
    for (let col = 0; col < BOARD_SIZE; col++) {
      // 오른쪽과 스왑
      if (col < BOARD_SIZE - 1) {
        const swapped = swapPieces(board, { row, col }, { row, col: col + 1 });
        if (findAllMatches(swapped).length > 0) return true;
      }
      // 아래와 스왑
      if (row < BOARD_SIZE - 1) {
        const swapped = swapPieces(board, { row, col }, { row: row + 1, col });
        if (findAllMatches(swapped).length > 0) return true;
      }
    }
  }
